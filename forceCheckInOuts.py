...
def activityOffset(userID): # Alex only has 1 activity registered, no need for an offset. Offset is 1-indexed based
    if userID == "582370335886802964": #Raf
        return 4
...
class Checkin:
    ...
    def checkin(self):
        ...
        if len(self.valid) >= 1 and len(self.invalid) == 0: # User only has valid activities
            print (f"User checked in for {self.valid} activities")
        elif len(self.valid) >= 1 and len(self.invalid) >= 1:  # User has both valid and invalid activities
            print (f"User checked in for {self.valid} activities, but also had invalid ones: {self.invalid}")
